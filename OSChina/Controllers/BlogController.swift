...
class BlogController: BaseButtonBarPagerTabStripViewController {
    
    var btnPublishPost: UIBarButtonItem?

    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "TITLE_BLOG".localized
    }

    override func childViewControllersForPagerTabStripViewController(pagerTabStripViewController: XLPagerTabStripViewController!) -> [AnyObject]! {
        let controller1 = BlogListController(flag: BlogListFlag.Latest)
        let controller2 = BlogListController(flag: BlogListFlag.Recommend)
